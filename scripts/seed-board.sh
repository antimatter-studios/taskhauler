#!/usr/bin/env bash
#
# Seed a "Taskhauler v2 Build" board with cards mined from
# frontend-v2-src/PARITY_CHECKLIST.md + BACKEND_GAPS.md.
#
# Eating our own dogfood: track the v2 implementation as cards in
# the very product we're building.
#
# Usage:
#   bash scripts/seed-v2-board.sh
#   API=http://taskhauler.localhost/api/v1 EMAIL=admin@taskhauler.localhost PASSWORD=admin bash scripts/seed-v2-board.sh
#
# Re-running: aborts if a board named "Taskhauler v2 Build" already exists.
...
BOARD_NAME="${BOARD_NAME:-Taskhauler v2 Build}"
...
BOARD_ID="$(api POST /boards "$(jq -n \
  ...
  --arg d "Dogfooding the v2 redesign. Cards mined from frontend-v2-src/PARITY_CHECKLIST.md and BACKEND_GAPS.md. Done = already shipped; In Progress = active or mocked; Review = needs verification; Backlog = not yet started or blocked on backend." \
...
card "$EP_P0" "$COL_DONE" medium task "frontend-v2,theme" \
...
card "$EP_P0" "$COL_DONE" low task "frontend-v2" \
...
card "$EP_P0" "$COL_DONE" medium task "frontend-v2,helpers" \
...
card "$EP_P0" "$COL_DONE" medium task "frontend-v2,primitives" \
...
card "$EP_P0" "$COL_DONE" medium task "frontend-v2,mocks" \
...
card "$EP_P0" "$COL_DONE" medium task "frontend-v2" \
...
card "$EP_P1" "$COL_DONE" medium task "frontend-v2,shell" \
...
card "$EP_P1" "$COL_DONE" low task "frontend-v2,shell" \
...
card "$EP_P2" "$COL_DONE" high task "frontend-v2,views" \
...
card "$EP_P2" "$COL_DONE" medium task "frontend-v2,views" \
...
card "$EP_P3" "$COL_DONE" medium task "frontend-v2,rail" \
...
card "$EP_P3" "$COL_INPROG" medium task "frontend-v2,rail,mocked" \
...
card "$EP_P4" "$COL_DONE" medium task "frontend-v2,views" \
...
card "$EP_P6" "$COL_INPROG" medium task "frontend-v2,presence,mocked" \
...
card "$EP_P7" "$COL_BACKLOG" medium task "frontend-v2,polish,a11y" \
  "Accessibility pass" \
...
card "$EP_P7" "$COL_BACKLOG" medium task "frontend-v2,polish" \
...
card "$EP_P8" "$COL_BACKLOG" high task "backend-v1,backend-gap" \
...
card "$EP_P8" "$COL_BACKLOG" medium task "backend-v1,backend-gap" \
...
card "$EP_P8" "$COL_BACKLOG" low task "backend-v1,backend-gap,infra" \
...
card "$EP_P8" "$COL_BACKLOG" low task "backend-v1,backend-gap" \
...
echo "    http://taskhauler-v2.localhost/    (then pick \"$BOARD_NAME\" from the sidebar)"
echo ""
